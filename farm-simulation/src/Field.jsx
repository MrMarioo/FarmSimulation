...
const Field = ({ field, onPlant, onWater, onHarvest }) => {
  return (
    <div style={{ border: '1px solid black', padding: '10px', width: '150px' }}>
      ...
      {!field.readyToHarvest ? (
        <>
          ...
          <button onClick={() => onPlant(field.id, 'Pszenica', 3)}>Zasadź Pszenicę</button>
          <button onClick={() => onPlant(field.id, 'Kukurydza', 5)}>Zasadź Kukurydzę</button>
        </>
      ) : (
